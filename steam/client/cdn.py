...
from compression.zstd import decompress as ZSTDDecompress
import lzma
...
class CDNClient:
    ...
    def get_cdn_auth_token(self, app_id: int, depot_id: int, hostname: str) -> dict:
        ...
        if self.steam.logged_on is False:
            return {
                'eresult': EResult.NotLoggedOn,
                'token': '',
                'expiration_time': 0
            }
    ...
    def get_chunk(self, app_id, depot_id, chunk_id):
        ...
        if (depot_id, chunk_id) not in self._chunk_cache:
            ...
            if data[:2] == b'VZ':
                ...
                if data[-3:] != b'zsv':
                    raise SteamError("VSZ: Invalid footer: %s" % repr(data[-3:]))
                if data[3:4] != b'a':
                    raise SteamError("VSZ: Invalid version: %s" % repr(data[3:4]))
                ...
                if crc32(data) != crc32_header != crc32_footer:
                    raise SteamError("VSZ: CRC32 checksum doesn't match for decompressed data")
            else:
